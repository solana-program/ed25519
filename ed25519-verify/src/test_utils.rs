...
/// Deserializes the 8-byte little-endian wire format.
fn read_offsets(input: &[u8]) -> SignatureOffsets {
    SignatureOffsets {
        signature_offset: u16::from_le_bytes(input[0..2].try_into().unwrap()),
        public_key_offset: u16::from_le_bytes(input[2..4].try_into().unwrap()),
        message_data_offset: u16::from_le_bytes(input[4..6].try_into().unwrap()),
        message_data_size: u16::from_le_bytes(input[6..8].try_into().unwrap()),
    }
}
